...
cmd.version('0.3.0')
    ...
    .option(
        '-e, --clear',
        'clears all data from the table, you must specify the --id'
    )
    .option('-g, --get', 'gets a dataset, you must specify the --id')
    .option('-i, --id <value>', 'specifies a dataset id')
    .option(
        '-a, --add <n>',
        'add n rows to the Customer table, you must specify the --id'
    )
    .parse(process.argv);
...
if (cmd.hasOwnProperty('consent')) {
    ...
    const context = new adal.AuthenticationContext(
        authority + directory + '/oauth2/authorize'
    );
        ...
        (error, tokenResponse) => {
            if (!error) {
                ...
                if (cmd.hasOwnProperty('create')) {
                    ...
                    cmd.hasOwnProperty('add') &&
                    cmd.hasOwnProperty('id')
                ) {
                    // generate the rows
                    const rows = [];
                    ...
                    request.post(
                        ...
                        (error, response, body) => {
                            if (!error && response.statusCode == 200) {
                                ...
                                if (error) {
                                    console.error(`error(103): ${error}`);
                                } else {
                                    console.error(
                                        `error(104) [${response.statusCode}]: ${
                                            response.statusMessage
                                        }`
                                    );
                                    console.log(body);
                    ...
                    request.get(
                        ...
                        (error, response, body) => {
                            if (!error && response.statusCode == 200) {
                                ...
                                if (error) {
                                    console.error(`error(105): ${error}`);
                                } else {
                                    console.error(
                                        `error(106) [${response.statusCode}]: ${
                                            response.statusMessage
                                        }`
                                    );
                                    console.log(body);
                    ...
                    request.delete(
                        ...
                        (error, response, body) => {
                            if (!error && response.statusCode == 200) {
                                ...
                                if (error) {
                                    console.error(`error(107): ${error}`);
                                } else {
                                    console.error(
                                        `error(108) [${response.statusCode}]: ${
                                            response.statusMessage
                                        }`
                                    );
                                    console.log(body);
